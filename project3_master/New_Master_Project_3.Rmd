...
We thought of building some interactive visualization and used Shiny (http://shiny.rstudio.com/) to accomplish the same. 
We chose four variables: Age,Country,GenderSelect,EmploymentStatus to consruct a subset. We kept Gender and Country as filter attributes to make data selection and filter the subset of data in the runtime. 

```{r niteends01, echo=FALSE,warning=FALSE, cache=FALSE, message=FALSE}

inputPanel(
  selectInput(inputId = "y1", 
                  label = "Gender:",
                  choices = c('Male','Female','All'),
                  #choices = trimws(as.character(survey.dem.plot$GenderSelect)), 
                  selected ='' ),
      
      # Select variable for x-axis
      selectInput(inputId = "x1", 
                  label = "Country:",
                  choices = trimws(as.character(survey.dem.plot$Country)), 
                  selected = '')
...
renderPlot({
    ...
    gendeSelect <- input$y1
    if(gendeSelect!='All'){
    data.survey.dem <- filter(survey.dem.plot,Country==input$x1,GenderSelect==input$y1,Age>5)
    }else{
    data.survey.dem <- filter(survey.dem.plot,Country==input$x1,Age>5)
    }
    print(median(data.survey.dem$Age))
    ggplot(data.survey.dem, aes(x = Age, y = data.survey.dem$count))+
      geom_bar(stat='summary',fun.y = "mean", aes(fill=EmploymentStatus))+
      geom_line(aes(x = median(Age), y = (data.survey.dem$count)))+
...
For country selection United States, the shiny plot clearly shows that full time employed users are the highest repondents and the median age for US population stays at 38 whereas the number of respondents peak for th age group 25. India appears to rank 2nd  with median age group 31.
...
Kaggele successfully captured repondents data and also clearly catetgaorized them into different time spent activities. In order to analyze this question we looked at attributes: TimeGatheringData,TimeModelBuilding,TimeProduction,,TimeVisualizing,
,TimeFindingInsights.
...
Whether one is employed full time, part time or a student; its worth exploring how people are using different learning platform and how they feel about it. We made use of different learning platform attributes captured in the dataset which also includes Kaggle as a learning platform.
...
subset %>%
        ...
        geom_bar(mapping = aes(x = MLMethodNextYearSelect, fill = MLMethodNextYearSelect)) +
                 ...
                 labs (x="ML Method ", y="count " , title="Formal Education vs ML/DS method excited about learning next year")
```
```{r, echo=FALSE}
result <- subset %>%
...
datatable(result, colnames=c("Formal Education", "ML/DS method excited about learning next year", "Percent"),class = 'cell-border stripe',caption = 'Table 1: Descriptive Statistics',options = list(pageLength = 8, dom = 'tip'))
```
...
1. What are the most frequently used data science (DS) methods by those writing code in DS professions? Do those relate to formal educational attainment?

2. What are the most frequently used DS methods? Where is the most time spent in terms of working with data? Do either of these correlate with job title or level of education?
