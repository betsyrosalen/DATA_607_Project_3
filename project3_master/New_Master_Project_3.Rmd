...
```{r figds01, fig.height = 5, fig.width = 7, fig.align = "center" , message= FALSE, echo=FALSE,warning=FALSE,}
library(ggplot2)
ggplot(data = survey.dem.plot, 
       mapping = aes(x = Age, fill = GenderSelect, 
                     y = ifelse(test = GenderSelect == "Male", 
                                yes = -count, no = count)))+
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = abs, limits = max(survey.dem.plot$count) * c(-1,1)) +
  labs(y = "Count") +
  coord_flip()+
  theme_bw()
...
The day typically starts with a question or business problem and involves following activities/ tasks:
...
```{r niteen04, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE}
...
```{r fig02, echo=FALSE, fig.align="center", fig.height=7, fig.width=8, message=FALSE}
...
Keeping in mind that Python is on the whole more likely to be recommended by Kaggle survey takers who use it alone or in tandem with R, the following question arises: is Python or R most profitable overall for those who are being paid in US Dollars?
...
+ *Time Series Analysis* as the third ML/DS method. High school graduates want to learn Genetic & Evolutionary Algorithms as theri third choice. 
