...
Variables: GenderSelect & Age <br>
Analysis: It appears that out of 16716 global Kaggle respondents there are 13610 males (84%) and 2778 females (16%). In this subset male respondents are almost 5 (~4.8) times more than female respondents. Also, from the plot below it is pretty evident that repondents' age peaks at 25 for both males and females whereas the median age for overall population is about 30.
...
Kaggle successfully captured repondents' data about time spent in different activities. <br>
Key Attributes: TimeGatheringData, TimeModelBuilding, TimeProduction, TimeVisualizing, and TimeFindingInsights.

## Exploratory Data Analysis (EDA)

After analyzing data for US repondents it appears that data cquisition  or gathering data is the main activitiy, at 37.75%.  This is where data scientists spent most of their time. Model building ranks 2nd, at 19.23%, followed by time spent in finding insights and data visualization. Only 10.23% of their total time appears to be taken by production activities. 
...
In order to determining usefulness of learners platfom we tidy the data for 18 learning platform attributes present in the data set and perform the analysis on long data type.  <br>
...
```{r}
#checking
datatable(df6, colnames=c("Programming Language Survey takers use", "Programming Language Survey takers recommend"
                          ,"Count", "Percent"),class = 'cell-border stripe',caption = 'Table 2: Descriptive Statistics',options = list(pageLength = 4, dom = 'tip'))
```
