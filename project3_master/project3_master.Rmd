...
survey takers, Bayesian Methods is also their third preference.
```
...
employed.WT <- employed[,39:86] <- lapply(employed[,39:86], flevels2)
...
learner.LP <- likert(learner[12:28], nlevels = 3)
learner.JS <- likert(learner[31:43], nlevels = 3)

employed.LP <- likert(employed[14:31], nlevels = 3)
employed.WT <- likert(employed[39:89], nlevels = 3)
employed.WM <- likert(employed[91:123], nlevels = 4)
...
# Code copied from here: http://statisfactions.com/2012/improved-net-stacked-distribution-graphs-via-ggplot2-trickery/
# and then modified so it would not remove the neutral column
...
net_stacked <- function(x) {
  ...
  positives <- setdiff(all_levels, c(negatives))
...
### Learning Platform Usefulness for Data Science Learners

```{r}
net_stacked(learner.LP)
```

### Learning Platform Usefulness for Employed Data Scientists

```{r}
net_stacked(employed.LP)
```

### Job Skill Importance for Data Science Learners

```{r}
net_stacked(learner.JS)
```

### Work Tools Frequency for Employed Data Scientists

```{r}
net_stacked(employed.WT)
```

### Work Methods Frequency for Employed Data Scientists
